...
export function validatePath(req: Request, res: Response, next: NextFunction): void {
  ...
  for (const candidate of candidates) {
    ...
    if (!resolved.startsWith(config.DOCS_ROOT + path.sep) && resolved !== config.DOCS_ROOT) {
...
export function requirePathQuery(req: Request, res: Response, next: NextFunction): void {
...
export function requirePathBody(req: Request, res: Response, next: NextFunction): void {
